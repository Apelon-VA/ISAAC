...
class IsDescendantOfSearchResultsFilter implements SearchResultsFilter {
	...
	public Collection<CompositeSearchResult> filter(Collection<CompositeSearchResult> results) throws SearchResultsFilterException {
        ...
        try {
			SearchResultsFilterFactory.LOG.debug("Applying " + (filter.getInvert() ? "Not(ConceptIsDescendentOf())" : "ConceptIsDescendentOf()") + filter + " to " + finalInputNids.size() + " nids");

			outputNids = q.compute();
			
			SearchResultsFilterFactory.LOG.debug(outputNids.size() + " nids remained after filtering a total of " + finalInputNids + " nids");
		} catch (Exception e) {
