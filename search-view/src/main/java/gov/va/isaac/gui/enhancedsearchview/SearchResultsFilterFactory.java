...
public class SearchResultsFilterFactory {
	private static final Logger LOG = LoggerFactory.getLogger(SearchResultsFilterFactory.class);
	...
	private static class IsDescendantOfSearchResultsFilter implements SearchResultsFilter {
		...
		public Collection<CompositeSearchResult> filter(Collection<CompositeSearchResult> results) throws SearchResultsFilterException {
	        ...
	        try {
				LOG.debug("Applying " + (filter.getInvert() ? "Not(ConceptIsDescendentOf())" : "ConceptIsDescendentOf()") + filter + " to " + finalInputNids.size() + " nids");

				outputNids = q.compute();
				
				LOG.debug(outputNids.size() + " nids remained after filtering a total of " + finalInputNids + " nids");
			} catch (Exception e) {
