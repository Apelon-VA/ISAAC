...
public abstract class AbstractPerMatchLuceneSearchStrategy<T> extends AbstractLuceneSearchStrategy<T> {
    private static final Logger LOG = LoggerFactory.getLogger(AbstractPerMatchLuceneSearchStrategy.class);

	public AbstractPerMatchLuceneSearchStrategy() {
		super();
	}
	public AbstractPerMatchLuceneSearchStrategy(List<T> resultsList) {
		super(resultsList);
	}
	...
	public synchronized void search() {
		...
		try
		{
					...
					if (resultCount > 0)
					{
						...
						for (SearchResult searchResult : searchResults) {
							...
							if (cc instanceof DescriptionAnalogBI)
							{
								...
								//int descTypeNid = ((DescriptionAnalogBI<?>) cc).getTypeNid();
							}
							else
							{
			...
			for (CompositeSearchResult result : tempUserResults) {
