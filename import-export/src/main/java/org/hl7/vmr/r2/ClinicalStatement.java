...
/**
 * A record of something of clinical relevance that is being done, has been
 * done, can be done, or is intended or requested to be done. A class that
 * serves as the basis for other more specific clinical statements, such as
 * ObservationEvent and ProcedureProposal. It is a concrete class that can be
 * used as is or specialized as needed.
 * 
 * Naming and modeling conventions:
 * 
 * - in general, attribute names end in 'Code' if and only if the name of the
 * attribute overlaps with the name of the parent class
 * 
 * 
 * - times are named as follows: Time is the default suffix for these
 * attributes. EventTime is used to distinguish the time an order is placed vs.
 * when the ordered act should take place. EffectiveTime and TimeInterval are
 * used when there is a desire to emphasize that a prolonged time interval
 * (e.g., > 1 day) can be used rather than a point in time or a short time
 * interval. Note that regardless of the naming convention, IVL_TS attributes
 * allow time intervals of any length.
 * 
 * 
 * - subjectEffectiveTime is the time that is primarily related to the subject's
 * experience of disease or treatment events (or durations), rather than when
 * those events were reported or recorded by the performer
 * 
 * 
 * - performerEventTime is the event time that is primarily related to the
 * performer, rather than the subject.
 * 
 * 
 * - the state between ordering and the ordered event occurring is modeled only
 * in cases of procedures and encounters, due to the substantial rate at which
 * orders do not result in events.
 * 
 * 
 * Approaches to representing specific statements:
 * 
 * - No known allergies --> NoKnownAllergy with conditionCode reflecting the
 * type of 'No known allergy' statements (e.g., 'No known allergy', 'No known
 * medication allergy', 'No known food allergy')
 * 
 * 
 * - No known medications --> UndeliveredSubstanceAdministration with substance
 * that is the root-level code for medications.
 * 
 * 
 * - No known problems --> DeniedProblem with conditionCode that is the
 * root-level code for problems or conditions.
 * 
 * 
 * - Patient takes an unknown drug --> SubstanceAdministrationEvent where code
 * for substance represents "unknown medication".
 * 
 * <p>
 * Java class for ClinicalStatement complex type.
 * 
 * <p>
...
@XmlType(name = "ClinicalStatement", propOrder = {
    "templateId", "id", "dataSourceType", "documentationTime",
    "evaluatedPersonId", "attribute", "comment", "relatedEntity",
    "relatedClinicalStatement"
})
@XmlSeeAlso({
    AdverseEventBase.class, OralDietBase.class, SupplyBase.class,
    ConditionBase.class, ObservationBase.class, ProcedureBase.class,
    GroupingClinicalStatement.class, GoalBase.class,
    SubstanceClinicalStatementBase.class, EncounterBase.class,
...
public class ClinicalStatement {
   ...
   * Therefore any modification you make to the returned list will be present
   * inside the JAXB object. This is why there is not a <CODE>set</CODE> method
   * for the relatedEntity property.
   * 
   * <p>
   * For example, to add a new item, do as follows:
   * 
   ...
   * Therefore any modification you make to the returned list will be present
   * inside the JAXB object. This is why there is not a <CODE>set</CODE> method
   * for the relatedClinicalStatement property.
   * 
   * <p>
   * For example, to add a new item, do as follows:
   * 
