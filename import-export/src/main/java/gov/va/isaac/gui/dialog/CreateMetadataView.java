...
public class CreateMetadataView implements PopupViewI, IsaacViewWithMenusI {
  ...
  public void showView(Window parent) {
    // TODO should really pop up a window with a progress bar, cancel button,
    // etc....
    ...
    Task<Boolean> task = new Task<Boolean>() {
      ...
      protected void succeeded() {
        ...
        if (metadataCreated) {
          AppContext.getCommonDialogs().showInformationDialog("Success",
              "Successfully created metadata.");
        } else {
          AppContext.getCommonDialogs().showInformationDialog("Info",
