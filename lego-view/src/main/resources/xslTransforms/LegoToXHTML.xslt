...
<!-- A XSL transform which turns a (valid) Lego XML file into XHTML 1.1 HTML files.  Internal formatting of the output is poor - 
...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	.indent
	{
		...
		border-width:1px;
				...
				<title>
					Lego <xsl:value-of select="lego/legoUUID"/>
				</title>
			</head>
			<body>
				<div class="title">Lego '<xsl:value-of select="lego/legoUUID"/>'</div>
				<div class="indent">
						<xsl:value-of select="lego/pncs/@name"/><br/> 
					<table>
						<tr>
							<td style="min-width: 200px;">Value: <xsl:value-of select="lego/pncs/@value"/></td>
							<td style="min-width: 200px;">ID: <xsl:value-of select="lego/pncs/@id"/></td>
						</tr>
						<tr>
							<td>Author: <xsl:value-of select="lego/stamp/@author"/></td>
							<td>Time: <xsl:value-of select="lego/stamp/@time"/></td>
						</tr>
						<tr>
							<td>Status: <xsl:value-of select="lego/stamp/@status"/></td>
							<td>Module: <xsl:value-of select="lego/stamp/@module"/></td>
						</tr>
						<tr>
							<td>Path: <xsl:value-of select="lego/stamp/@path"/></td>
						</tr>
					</table>
					<xsl:for-each select="lego/assertion">
