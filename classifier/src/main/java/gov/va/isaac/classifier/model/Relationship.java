...
/**
 * The Class Relationship. Represents a relationship.
 */
public class Relationship implements Comparable<Object> {
   ...
   */
  public Relationship(int sourceId, int destinationId, int roleTypeId,
      int group, String relationshipId) {
  ...
  public String toString4File() {
    return "relId     \t" + "sourceId      \t" + "destinationId      \t"
        + "typeId    \t" + "group";
  }
    ...
    try {
      ...
      if (format == 1) { // RAW NIDs
        for (Relationship sr : srl) {
          bw.write(sr.sourceId + "\t" + sr.typeId + "\t" + sr.destinationId
              + "\t" + sr.group + "\r\n");
        }
      }
      ...
      if (format == 6) { // Distribution Form
        ...
        for (Relationship sr : srl) {
          ...
          // CONCEPTID2
          bw.write("#" + index + "\t" + sr.sourceId + "\t" + sr.typeId + "\t"
              + sr.destinationId + "\t");
