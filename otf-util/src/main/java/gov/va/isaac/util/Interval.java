...
public class Interval
{
	...
	public Interval(String parseFrom)
	{
		...
		if (pos == 0)
		{
			...
			right_ = NumberUtilities.parseNumber(numeric.substring(1, numeric.length()));
		}
		else if (pos > 0)
		{
			left_ = NumberUtilities.parseNumber(numeric.substring(0, pos));
			if (numeric.length() > (pos + 1))
			{
				right_ = NumberUtilities.parseNumber(numeric.substring(pos + 1));
			}
		}
		else
		{
		...
		{
			if (NumberUtilities.compare(left_, right_) > 0)
			{
